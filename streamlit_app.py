...
#analysis
from analysis.data_cleaning import clean_join_df, join_dataframes
from analysis.transform import group_and_calculate_percentage, group_and_calculate_percentage_with_names, join_and_rename_columns, group_and_get_filtered_max_value, group_and_get_filtered_min_value, join_dataframes_jaro,filter_dataframes_jaro, get_combined_total_ges, sum_top_n_elements
...
if choice == "Acceuil":
    ...
    with tab1:
        ...
        with st.container(border=True) :
            ...
            df_eval, df_eval_contributeurs = upload_eval()
            df_explo, df_explo_contributeurs = upload_explo()

        with st.container(border=True) :

            if df_eval is not None and df_explo is not None:
                df_eval = clean_join_df(df_eval, df_eval_contributeurs)
                df_explo = clean_join_df(df_explo, df_explo_contributeurs)
    ...
    with tab2:
        ...
        if df_eval is not None and df_explo is not None:
            ...
            jaro_join_df = join_dataframes_jaro(df_eval, df_explo)
            ...
            with st.container(border=True) :
                ...
                max_eval = group_and_get_filtered_max_value(df_eval, 'Lot', 'GES_divided_by__SREF', selected_option)
                max_explo = group_and_get_filtered_max_value(df_explo, 'Lot', 'GES_divided_by__SREF', selected_option)
                y_axis_lots = round(max(max_eval, max_explo)*1.2,0)
                ...
                #LOT STACKED BARS ------------------------- Hacer esto un py file o muchas funciones para reutilizar en sous-lots
                #creation of select for lot
                sous_lot_select_options = (filtered_exploration_eval['Sous-lot'].unique())
                # Set the initial option as a string value
                initial_option = sous_lot_select_options[0]
                # Create the select box with the initial option set by value
                sous_lot_selected_option = st.selectbox("Select a Sous-lot", sous_lot_select_options, index=sous_lot_select_options.tolist().index(initial_option))

                # Convert the selected option to a list
                sous_lot_selected_option_list = [sous_lot_selected_option]
                #getting max values of lots for the y axis scale 
